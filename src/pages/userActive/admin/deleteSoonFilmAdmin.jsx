...
import { deleteMovieSoon, getAllFilmSoon } from "../../../utils/apiFilmSoon";
...
function DeleteFilmAdmin() {
    ...
    const deleteData = (idSoonFilm, nameFilm) => {
        Swal.fire({
            title: "¿Estás seguro?",
            ...
            if (result.isConfirmed) {
                deleteMovieSoon(idSoonFilm, nameFilm)
                    .then(() => {
                        getFilm();
                    })
                    .catch((error) => {
            ...
            <table className="table table-bordered" id="tableData">
                ...
                <tbody>
                    {filteredFilm.map((film) => (
                        <tr key={film.idSoonFilm}>
                            <td>{film.idSoonFilm}</td>
                            <td>{film.codeFilm}</td>
                            <td>{film.nameFilm}</td>
                            <td>
                                <button className="btn btn-danger" onClick={() => deleteData(film.idSoonFilm, film.nameFilm)}>Eliminar</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
